<div class="modal" id="inventory" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="btn" id="inventory" style="color: black;">Inventory</h5>
                <h5 class="btn" id="filter" style="color: black;">Mob Drops</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="inventory">
                    <form method="post">
                        <?php foreach ($character->getInventory() as $key => $value) { ?>
                            <button class="btn btn-success" type="submit" name="btn_use_item"
                                    value="<?php echo $key ?>"><?php echo $key . ": ". $value ?></button>
                        <?php } ?>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>

<?php
if(isset($_POST['btn_use_item'])){
    $character->useItem($_POST['btn_use_item']);
}

?>

<style>
    .inventory button{
        margin: 8px;
    }
</style>